...
export async function getStats(): Promise<PortfolioStats> {
  ...
  // Update position values with current prices (simulate drift)
  const positions = portfolio.positions.map(p => {
    const drift = (Math.random() - 0.48) * 0.02;
    const current = Math.max(0.01, Math.min(0.99, p.current + drift));
    const pnl = (current - p.entry) * p.shares * (p.direction === 'YES' ? 1 : -1);
    const pnlPct = ((current - p.entry) / p.entry) * 100 * (p.direction === 'YES' ? 1 : -1);
