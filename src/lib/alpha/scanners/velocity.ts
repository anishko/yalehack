  ...
  for (const market of markets) {
    ...
    if (absChange > 0.05) {
      ...
      signals.push({
        ...
        direction,
        confidence: Math.round(Math.min(90, 40 + Math.log1p(momentum) * 12 + (isAccelerating ? 8 : 0))),
        expectedEdge: absChange * 0.5,
        riskScore: Math.max(25, Math.round(55 + Math.min(25, momentum))),
        edgeScore: Math.round(Math.min(3.5, Math.log1p(absChange * 10) * 1.2 + (isAccelerating ? 0.3 : 0)) * 100) / 100,
