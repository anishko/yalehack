    ...
    for (const market of linked) {
      ...
      if (!hasReacted) {
        ...
        signals.push({
          ...
          confidence: Math.round(Math.min(88, 42 + magnitude * 4 + Math.log1p(magnitude) * 8)),
          expectedEdge: Math.min(0.15, magnitude * 0.01),
          riskScore: Math.max(20, Math.round(60 - magnitude * 3)),
          edgeScore: Math.round(Math.min(3.0, Math.log1p(magnitude) * 0.8 + magnitude * 0.08) * 100) / 100,
